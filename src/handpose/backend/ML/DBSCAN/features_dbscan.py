...
class DbscanFeatureExtractor:
    def __init__(self) -> None:
        """
        Initialisierung des Feature-Extractors für DBSCAN.
        """
        pass

    def process_csv(self, file_path: str) -> pd.DataFrame | None:
        """
        Lädt und bereinigt eine CSV-Datei mit Keypoint-Daten.
    ...
    def calculate_features(self, df: pd.DataFrame) -> tuple[np.ndarray, None]:
        """
        Berechnet Features für die HMM-Vorfilterung.

        Args:
            df (pd.DataFrame): DataFrame mit Keypoint-Daten.

        Returns:
            tuple: (X, None) - X ist NumPy-Array mit berechneten Features.
        """
        temp_data = []
